...
def fetchArtists(id):
    url = 'http://music.163.com/api/artist/list?offset=0&limit=100&categoryCode={}'.format(id)
    r = requests.get(url, headers=headers)
    return json.loads(r.content)

def fetchMusics(artistId):
    url = 'http://music.163.com/api/v1/artist/{}'.format(artistId)
    r = requests.get(url, headers=headers)
    return json.loads(r.content)['hotSongs']
